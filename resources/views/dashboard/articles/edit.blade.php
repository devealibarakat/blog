            ...
            <form action="{{ route('articles.update', $article) }}" method="POST">
                ...
                <div class="tab-content mt-3 p-3 row">
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $properties)
                        <div class="tab-pane fade show {{ $loop->first ? 'active' : '' }}" id="{{ $locale }}">
                            <div class="row pt-3">
                                ...
                                <div class="col-lg-6 p-2">
                                    <label for="slug_{{ $locale }}" class="form-label">{{ __('inputs.slug.label') }}
                                        ({{ $properties['native'] }})</label>
                                ...
                                <div class="col-lg-6 p-2">
                                    ...
                                    <input type="text" class="form-control" name="short_description[{{ $locale }}]"
                                        ...
                                        value="{{$article->translate($locale)->short_description}}">
                                ...
                                <div class="col-lg-6 p-2">
                                    ...
                                    <input type="text" class=" form-control" name="meta_keywords[{{ $locale }}]"
                                        ...
                                        value="{{ implode(',', (array) $article->translate($locale)->meta_keywords) }}">
                                ...
                                <div class="col-lg-12 p-2">
                                    ...
                                    <textarea class="form-control" name="description[{{ $locale }}]"
                                        id="description{{ $locale }}" rows="5"></textarea>
                    ...
                    <div class="col-lg-6 p-2">
                        ...
                        <select name="tags[]" multiple class="form-control  border" required>
                            @foreach($tags as $tag)
                                <option value="{{ $tag->id }}" {{ $article->tags->contains($tag->id) ? 'selected' : '' }}>
                                    {{ $tag->translate($locale)->name }}
                                </option>
                            @endforeach
                        </select>
